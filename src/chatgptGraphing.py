...
for wf in unique_waveforms:
  ...
  D = -B
  kurt.append((6 * A) / (D**2))
  md.append(D)  # Removed the * 1000 multiplier to keep true physical units
  curvesA.append(A)
  curvesB.append(B)
  curvesC.append(C)
  ...
  ax.plot(
      ...
      y_fit,
      linestyle="--",
      label=f"Fit (MD: {md[-1]:.4e}, K: {kurt[-1]:.4f})",
  )
...
ax.scatter(
    freq, diff, color="red", marker="o", s=10, zorder=5, label="MD Data Points"
)
